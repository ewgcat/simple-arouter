...
@SupportedAnnotationTypes({com.lishuaihua.arouter_compiler.utils.ProcessorConfig.PARAMETER_PACKAGE}) // 我们服务的注解
...
public class ParameterProcessor extends AbstractProcessor {
    ...
    public boolean process(Set<? extends TypeElement> set, RoundEnvironment roundEnvironment) {
        ...
        if (!com.lishuaihua.arouter_compiler.utils.ProcessorUtils.isEmpty(set)) {
            // 获取所有被 @Parameter 注解的 元素（属性）集合
            Set<? extends Element> elements = roundEnvironment.getElementsAnnotatedWith(Parameter.class);

            if (!com.lishuaihua.arouter_compiler.utils.ProcessorUtils.isEmpty(elements)) {
                // 给仓库 存储相关信息
                ...
                TypeElement activityType = elementUtils.getTypeElement(com.lishuaihua.arouter_compiler.utils.ProcessorConfig.ACTIVITY_PACKAGE);
                TypeElement parameterType = elementUtils.getTypeElement(com.lishuaihua.arouter_compiler.utils.ProcessorConfig.AROUTER_AIP_PARAMETER_GET);

                // 生成方法
                // Object targetParameter
                ParameterSpec parameterSpec = ParameterSpec.builder(TypeName.OBJECT, com.lishuaihua.arouter_compiler.utils.ProcessorConfig.PARAMETER_NAME).build();
